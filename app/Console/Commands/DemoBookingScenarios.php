...
class DemoBookingScenarios extends Command
{
     ...
     * ===========================================================================
     *
     * Демонстрация работы с фиксированными слотами и автоматическим подтверждением.
     * Особое внимание уделяется обработке перерывов и пограничным случаям.
     */
    private function runScenario1(array $setupData): void
    {
        ...
        $this->line("   Цель: Показать пользователю доступные временные интервалы");
        ...
        $this->line("   Цель: Проверить бронирование непосредственно перед перерывом");
        ...
        $this->line("   Цель: Проверить валидацию пересечений с перерывами");
        try {
            $this->createBooking([
                'resource_id' => $resourceId,
        ...
        $this->info("\n✅ ШАГ 4: Бронь первого слота после перерыва...");
        $this->line("   Цель: Проверить бронирование сразу после перерыва");
        ...
        $this->line("   Цель: Тестирование различных временных диапазонов относительно перерыва");
        $this->testBoundaryCases($resourceId);

        // ШАГ 6: Отмена брони клиентом
        $this->info("\n🔄 ШАГ 6: Отмена брони клиентом...");
        $this->line("   Цель: Проверить процесс отмены в разрешенное время");
        ...
        // ШАГ 7: Перенос брони клиентом
        $this->info("\n🔄 ШАГ 7: Перенос брони клиентом...");
        $this->line("   Цель: Проверить процесс переноса в разрешенное время");
     ...
     * ===========================================================================
     *
     * Демонстрация работы с динамическими слотами и ручным подтверждением.
     * Показывает разницу в правах администратора и обычного пользователя.
     */
    private function runScenario2(array $setupData): void
    {
        ...
        $this->line("🧪 Тестирование: права администратора, ожидание подтверждения, динамические слоты");

        // ШАГ 1: Администратор создает бронь вне стандартного расписания
        $this->info("\n👨‍💼 ШАГ 1: Администратор создает бронь вне расписания...");
        $this->line("   Цель: Показать привилегии администратора (обход ограничений)");
        ...
        $this->info("\n👤 ШАГ 2: Пользователь создает бронь (требует подтверждения)...");
        $this->line("   Цель: Показать процесс ручного подтверждения для пользователей");
        ...
        $this->line("   Цель: Показать как pending брони влияют на доступность слотов");
        $slots = $this->getSlots($resourceId, '2024-01-16', 8);
        $this->info("   📊 Доступные слоты: " . count($slots));

        // ШАГ 4: Подтверждение брони администратором
        $this->info("\n✅ ШАГ 4: Подтверждение брони администратором...");
        $this->line("   Цель: Показать процесс ручного подтверждения администратором");
        ...
        $this->line("   Цель: Проверить временные ограничения на отмену");
        try {
     ...
     * ===========================================================================
     *
     * Демонстрация работы с групповыми ресурсами и ограничением участников.
     * Показывает как работают множественные bookers и лимиты.
     */
    private function runScenario3(array $setupData): void
    {
        ...
        $this->line("   Цель: Показать доступные слоты для групповых занятий");
        ...
        $this->line("   Цель: Создать основную бронь для группового занятия");
        ...
        $this->line("   Цель: Показать процесс добавления множественных участников");
        ...
        $this->line("   Цель: Проверить ограничение максимального количества участников");
        try {
            $this->addParticipantsToBooking($groupBooking['id'], [
        ...
        $this->line("   Цель: Показать как групповые брони влияют на доступность ресурса");
     ...
     * ===========================================================================
     *
     * Демонстрация работы с дорогими ресурсами с строгими ограничениями.
     * Показывает обязательное подтверждение и длительные сроки отмены/переноса.
     */
    private function runScenario4(array $setupData): void
    {
        ...
        $this->line("   Цель: Проверить ограничение минимального времени для брони");
        try {
            $this->createBooking([
                'resource_id' => $resourceId,
        ...
        $this->info("\n⏳ ШАГ 2: Корректная бронь с ожиданием подтверждения...");
        $this->line("   Цель: Показать процесс бронирования с обязательным подтверждением");
        ...
        $this->info("\n❌ ШАГ 3: Отклонение брони администратором...");
        $this->line("   Цель: Показать процесс отклонения брони администратором");
        ...
        $this->info("\n👨‍💼 ШАГ 4: Бронь администратором с обходом ограничений...");
        $this->line("   Цель: Показать привилегии администратора для срочных броней");
        $adminBooking = $this->createBooking([
            'resource_id' => $resourceId,
        ...
        $this->line("   Цель: Проверить строгие ограничения на отмену");
        try {
     ...
     * ===========================================================================
     *
     * Демонстрация работы с многодневными бронями.
     * Показывает как система обрабатывает брони, пересекающие несколько дней.
     */
    private function runScenario5(array $setupData): void
    {
        ...
        $this->line("   Цель: Создать многодневную бронь гостиничного номера");
        ...
        $this->line("   Цель: Проверить валидацию пересекающихся многодневных броней");
        try {
            $this->createBooking([
                'resource_id' => $resourceId,
        ...
        $this->info("\n✅ ШАГ 3: Бронь сразу после освобождения номера...");
        $this->line("   Цель: Показать бронирование сразу после предыдущей брони");
        ...
        $this->info("\n📅 ШАГ 4: Проверка доступности в диапазоне дат...");
        $this->line("   Цель: Показать проверку доступности для диапазонов дат");
        $this->testDateRangeAvailability($resourceId);

        // ШАГ 5: Перенос длительной брони администратором
        $this->info("\n🔄 ШАГ 5: Перенос длительной брони администратором...");
        $this->line("   Цель: Показать перенос многодневной брони");
     ...
     * ===========================================================================
     *
     * Демонстрация разницы в правах между администратором и пользователем.
     * Показывает приоритет администратора в экстренных ситуациях.
     */
    private function runScenario6(array $setupData): void
    {
        ...
        $this->info("\n👤 ШАГ 1: Пользователь создает обычную бронь...");
        $this->line("   Цель: Создать обычную пользовательскую бронь для последующего тестирования");
        $userBooking = $this->createBooking([
            'resource_id' => $resourceId,
        ...
        $this->line("   Цель: Показать приоритет администратора - бронь поверх существующей");
        ...
        $this->info("\n🔄 ШАГ 4: Администратор переносит экстренную бронь...");
        $this->line("   Цель: Показать возможность администратора переносить брони");
        ...
        $this->line("   Цель: Показать возможность пользователя забронировать освободившееся время");
     ...
     * ===========================================================================
     *
     * Демонстрация работы с праздничными днями и выходными.
     * Показывает как система учитывает нерабочие дни в расписании.
     */
    private function runScenario7(array $setupData): void
    {
        ...
        foreach ($testDates as $date => $info) {
            ...
            $this->line("   Цель: Проверить доступность слотов в разные типы дней");
        ...
        $this->line("   Цель: Проверить что перерывы правильно исключаются из доступных слотов");
     ...
     * ===========================================================================
     *
     * Демонстрация работы со сложным расписанием с множественными перерывами.
     * Показывает как система обрабатывает сложные временные паттерны.
     */
    private function runScenario8(array $setupData): void
    {
        ...
        $this->line("   Цель: Показать генерацию слотов при множественных перерывах");
        ...
        $this->line("   Цель: Показать бронирование в промежутке между перерывами");
        ...
        $this->info("\n✅ ШАГ 3: Подтверждение брони администратором...");
        $this->line("   Цель: Показать процесс подтверждения для сложных броней");
        ...
        $this->info("\n🔍 ШАГ 4: Попытка брони через несколько перерывов...");
        $this->line("   Цель: Проверить бронирование, охватывающее несколько перерывов");
        try {
        ...
        $this->info("\n🎄 ШАГ 5: Проверка доступности в праздничный день...");
        $this->line("   Цель: Проверить что в праздничные дни слотов нет");
    ...
    // ===========================================================================
    // API ВЫЗОВЫ К СИСТЕМЕ БРОНИРОВАНИЯ
    // ===========================================================================
